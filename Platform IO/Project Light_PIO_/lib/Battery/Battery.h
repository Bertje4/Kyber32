...
// ── Charging detection ────────────────────────────────────────────────────────
// The charging rail is monitored via an identical voltage divider (same R1/R2).
// A voltage above this threshold on the charging pin means the charger is live.
// Roughly equivalent to ~1 V on the raw rail after the divider — well above
// ADC noise but safely below any "dead charger" false-positive scenario.
#define CHARGING_DETECT_THRESHOLD_MV 2000   // millivolts at the ESP ADC pin ~> includes the vlatge divider
...
private:
  ...
  uint32_t       readChargingPin_mV();   // raw ADC reading on CHARGING_PIN
...
public:
  ...
  bool          isCharging();   // true when charger voltage detected on CHARGING_PIN
};
...
// True while a charger voltage is detected on CHARGING_PIN.
// Written by BatteryMonitor::BatteryCode(), read by all subsystems.
extern volatile bool charging_detected;
