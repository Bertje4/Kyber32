...
void Blade::LEDCode() {
  ...
  for (;;) {
    ...
    } else if (global_state == lightsaber_charging) {
      // Charge-safe mode: all LEDs off to minimise power draw.
      fill_solid(leds_output_array, TOTAL_LEDS, CRGB::Black);
#if NUM_LEDS_CRYSTAL > 0
      fill_solid(crystal_output_array, NUM_LEDS_CRYSTAL, CRGB::Black);
#endif
      FastLED.show();
      // Poll slowly — nothing to animate while charging.
