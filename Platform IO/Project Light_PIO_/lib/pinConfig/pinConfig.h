...
#define ON_PLATFORM_IO // on PlatformIO the PrintTasks() function does not excist. So if on PIO, leave uncommented
...
// **** UART Communication ****
#define RX_DFPLAYER 7
#define TX_DFPLAYER 6
...
leave line 25 uncommented and don't worry about every thing with SECOND_ as these definitions will not be used, just fill in MAIN_.

If you do want to use a second button, comment line 25 and make sure both MAIN_ and SECOND_ settings are correct
*/
...
/*
Button LED wiring — two options per button:

  RGB button:  define <MAIN/SECOND>_BUTTON_RGB and set all three colour pins.
               The LED shows red (idle), blue (saber on), or green (config/web).

  Single-LED button:  leave <MAIN/SECOND>_BUTTON_RGB commented out and set only
                      <MAIN/SECOND>_LED.  The LED will simply turn ON whenever the
                      saber is active (on, config, or web-config) and OFF when idle.
*/

// #define MAIN_BUTTON_RGB   // comment out for a non-RGB (single-LED) main button
#define MAIN_BUTTON_WIRED_TO_3V3 // in the standard build the main button is wired to 3v3. Comment this out in case it is wired to GND
#define BUTTON_LEDS_WIRED_TO_3V3 // in the standard build the LEDS in the buttons are wired to 3V3, so the esp writes HIGH for off and LOW for on. This one line covers all the in button LEDS, they must all be wired in the same way!
  ...
  #define MAIN_BLUE  18
#else
  #define MAIN_LED   18   // single LED pin for non-RGB main button
#endif

#ifdef SINGLE_BUTTON_MODE
  #define LONG_PRESS_LOCKUP_MS 4000  // hold longer than this = lockup, shorter = retract
#endif

#ifndef SINGLE_BUTTON_MODE  // When using two buttons -> line 25 uncommented: make sure these pins are defined correctly
  #define SECOND_BUTTON 25
  #define SECOND_BUTTON_WIRED_TO_3V3 // in the standard build the main button is wired to 3v3. Comment this out in case it is wired to GND

  #define SECOND_BUTTON_RGB   // comment out for a non-RGB (single-LED) secondary button
  ...
  #else
    #define SECOND_LED   26   // single LED pin for non-RGB secondary button
  #endif
#endif

#ifdef MAIN_BUTTON_WIRED_TO_3V3
  #define MAIN_BUTTON_PULL_STATE INPUT_PULLDOWN
  #define MAIN_BUTTON_ACTIVE_STATE false
#else 
  #define MAIN_BUTTON_PULL_STATE INPUT_PULLUP
  #define MAIN_BUTTON_ACTIVE_STATE true
#endif

#ifndef SINGLE_BUTTON_MODE
  #ifdef SECOND_BUTTON_WIRED_TO_3V3
    #define SECOND_BUTTON_PULL_STATE INPUT_PULLDOWN
    #define SECOND_BUTTON_ACTIVE_STATE false
  #else 
    #define SECOND_BUTTON_PULL_STATE INPUT_PULLUP
    #define SECOND_BUTTON_ACTIVE_STATE true
  #endif
#endif
...
#define LED_OUTPUT 3   // pin of the LED strip all the strips must be connected to this pin, either in series or parallel
#define NUM_LEDS 118    // number OF LEDS of one strip: parallel: LEDS in strip going up
...
#define SABER_BRIGHTNESS 70  // percentage
...
For safety and longevity reasons, it is not recommended to discharge battery while charging it.
As I did not have any MOSFETs at hand, I use code to limit discharge rates.
While the battery is charging, voltage detected over the charging rails via a voltage divider with the same characteristics as the one from the battery.
the ESP will put the dfplayer into sleep and will keep it in sleep during the charge. At the same time,
the esp will allow nothing else: no wifi, nothing and it will just be checking if it is still charging.
When the voltages gets removed, the normal program will resume.

Also, when the charger gets plugged in while the saber is on (so blade on etc...) the esp will just turn off all the LEDS etc
and play the animations. All for the quickest possible shut down.
*/

#define R1 100000
#define R2 100000

#define BATTERY_PIN 2     // Pick a free ADC1 pin (ADC2 conflicts with WiFi)
#define CHARGING_PIN  4   // Pick a free ADC1 pin (ADC2 conflicts with WiFi)

// **** Miscellaneous: can be tweaked to preference but require knowledge of the software ****
// 2. Add task config constants
// #define DUAL_CORE // comment out when using a single-core soc
...
#define DFPLAYER_TASK_STACK_SIZE 6144
...
#define DEBUG_PRINT(x)   do { \
  if(serial_mutex != NULL) xSemaphoreTake(serial_mutex, portMAX_DELAY); \
  Serial.print(x); \
  if(serial_mutex != NULL) xSemaphoreGive(serial_mutex); \
} while(0)
